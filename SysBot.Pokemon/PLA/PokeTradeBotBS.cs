...
#pragma warning restore CA1031 // Do not catch general exception types
                    ...
                    if (offered.IsNicknamed)
                    {
                        if (offered.Nickname.Length == 6 && uint.TryParse(offered.Nickname[..3], out _))
                        {
                            ...
                            if (generatedMon != null)
                            {
                                ...
                                Hub.Queues.Enqueue(PokeRoutineType.PLALinkTrade, poke, 3);
                            }
                        }
                    }
                }
        ...
        private async Task<TradePartnerPLA> GetTradePartnerInfo(CancellationToken token)
        {
            ...
            var name = await SwitchConnection.PointerPeek(TradePartnerPLA.MaxByteLengthStringObject, Offsets.LinkTradePartnerNamePointer, token).ConfigureAwait(false);
