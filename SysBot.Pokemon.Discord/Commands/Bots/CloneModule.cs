...
namespace SysBot.Pokemon.Discord
{
    ...
    public class CloneModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
    {
        ...
        public async Task CloneAsync(int code)
        {
            ...
            await QueueHelper<T>.AddToQueueAsync(Context, code, Context.User.Username, sig, new T(), PokeRoutineType.PLAClone, PokeTradeType.Clone).ConfigureAwait(false);
        }
        ...
        public async Task CloneAsync([Summary("Trade Code")][Remainder] string code)
        {
            ...
            var sig = Context.User.GetFavor();
            await QueueHelper<T>.AddToQueueAsync(Context, tradeCode == 0 ? Info.GetRandomTradeCode() : tradeCode, Context.User.Username, sig, new T(), PokeRoutineType.PLAClone, PokeTradeType.Clone).ConfigureAwait(false);
        }
